...
func Test_ListAllVirtualNetwork_MultiplesResults(t *testing.T) {
	...
	c := &cache.MockCache{}
	c.On("Get", "ListAllVirtualNetwork").Return(nil).Times(1)
	c.On("Put", "ListAllVirtualNetwork", expected).Return(true).Times(1)
	s := &networkRepository{
		virtualNetworkClient: fakeClient,
		cache:                c,
	}
	got, err := s.ListAllVirtualNetwork()
	if err != nil {
		t.Errorf("ListAllVirtualNetwork() error = %v", err)
		return
	}
	...
	if !reflect.DeepEqual(got, expected) {
		t.Errorf("ListAllVirtualNetwork() got = %v, want %v", got, expected)
	}
}
...
func Test_ListAllVirtualNetwork_MultiplesResults_WithCache(t *testing.T) {
	...
	c := &cache.MockCache{}
	c.On("Get", "ListAllVirtualNetwork").Return(expected).Times(1)
	s := &networkRepository{
		virtualNetworkClient: fakeClient,
		cache:                c,
	}
